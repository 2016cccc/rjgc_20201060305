...
def get_new_dict(type_vec_path,type_word_path,final_vec_path,final_word_path):  #词标签，词向量
    ...
    # 输出词向量
    word_dict = ['PAD','SOS','EOS','UNK']#其中0 PAD_ID,1 SOS_ID,2 E0S_ID,3 UNK_ID
...
if __name__ == '__main__':
    ...
    path = '../hnn_process/embeddings/' + version + '/' + language_type + '_struc2vec1/data/' + language_type + '_struc2vec.txt' #239s
    path_bin = '../hnn_process/embeddings/' + version + '/' + language_type + '_struc2vec.bin' #2s


    if not os.path.exists(path_bin):
        trans_bin(path, path_bin)
    #113440 27970(2) 49409(12),50226(30),55993(98)

    #==========================  ==========最初基于Staqc的词典和词向量==========================

    word_path = '../hnn_process/data/word_dict/' + language_type + '_word_vocab_dict.txt'
    word_vec_path = '../hnn_process/embeddings/' + language_type + '/' + language_type + '_word_vocab_final.pkl'
    word_dict_path = '../hnn_process/embeddings/' + language_type + '/' + language_type + '_word_dict_final.pkl'



    #txt存储数组向量，读取时间：30s,以pickle文件存储0.23s,所以最后采用pkl文件
    if not os.path.exists(word_dict_path):
        get_new_dict(path_bin, word_path, word_vec_path, word_dict_path)
    ...
    #python最后的词典和对应的词向量
    final_word_vec_path = '../hnn_process/ulabel_data/large_corpus/' + language_type + '_word_vocab_final.pkl'
    final_word_dict_path = '../hnn_process/ulabel_data/large_corpus/' + language_type + '_word_dict_final.pkl'
    ...
    #处理成打标签的形式
    staqc_f ='../hnn_process/ulabel_data/staqc/seri_' + language_type + '_staqc_unlabled_data.pkl'
    Serialization(final_word_dict_path, new_staqc, staqc_f)

    print('' + language_type + ' 序列化完毕')
